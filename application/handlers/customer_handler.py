...
from application.services.customer_service import submit_user_question, register_user
...
def register(event, context):
    ...
    schema = {
        ...
        "required": ["wallet_address", "block_number"],
    }
    ...
    try:
        ...
        if inputs is None or signature is None:
            ...
            payload = json.loads(inputs)
            validate(instance=payload, schema=schema, format_checker=FormatChecker())
            statusCode, message = register_user(payload, signature)
    except ValidationError as e:
        message = e.message
