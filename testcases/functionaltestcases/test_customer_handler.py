...
class TestCustomerHandler(TestCase):
    ...
    def test_submit_question_with_valid_body(self):
        response = submit_question(event={"body": '{"email": "mail@mail.com"}',"headers": {"Authorization": "0x36980e7e7ccfe437a5b60ccf26221b2b9ff6f80f39548162d7764c7e7a0992256c42670244b21f5d367420c656aef86e9a164944f10d4f6e64d303c8b575a0d71b"}}, context=None)
        self.assert_(response['statusCode'] == HTTPStatus.BAD_REQUEST.value)

    def test_submit_question_with_invalid_body(self):
        response = submit_question(event={"body": '{"wallet_address": "0x12345", "comment": "test comment", "email": "mail@mail.com"}', "headers": {}}, context=None)
        self.assert_(response['statusCode'] == HTTPStatus.BAD_REQUEST.value)
